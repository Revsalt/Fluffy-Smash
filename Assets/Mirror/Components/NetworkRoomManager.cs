...
public class NetworkRoomManager : NetworkManager
{
    ...
    public Transform GetPositionWithTeam(NetworkConnection conn)
    {
        NetworkStartPosition[] poss = FindObjectsOfType<NetworkStartPosition>();

        Transform startPos = null;
        if (conn.identity.GetComponent<NetworkRoomPlayerExt>().Team == "Red")
        {
            foreach (var item in poss)
            {
                if (item.TeamName == "Red")
                {
    ...
    void SceneLoadedForPlayer(NetworkConnection conn, GameObject roomPlayer)
    {
        ...
        if (gamePlayer == null)
        {
            ...
            Transform startPos = GetPositionWithTeam(conn);
    ...
    public override void OnServerAddPlayer(NetworkConnection conn)
    {
        ...
        if (IsSceneActive(RoomScene))
        {
            ...
            int TeamID = UnityEngine.Random.Range(0, 2);

            newRoomGameObject.GetComponent<NetworkRoomPlayerExt>().Team = TeamID == 0 ? "Red" : "Blue";
