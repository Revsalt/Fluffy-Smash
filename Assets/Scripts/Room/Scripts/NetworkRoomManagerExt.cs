...
public class NetworkRoomManagerExt : NetworkRoomManager
{
    ...
    public override bool OnRoomServerSceneLoadedForPlayer(NetworkConnection conn, GameObject roomPlayer, GameObject gamePlayer)
    {
        ...
        gamePlayer.GetComponent<PlayerNetworkManager>().Team_m = roomPlayer.GetComponent<NetworkRoomPlayerExt>().team_m;
    ...
    public override void SceneLoadedForPlayer(NetworkConnection conn, GameObject roomPlayer)
    {
        ...
        if (gamePlayer == null)
        {
            ...
            startPos = NetworkStartPositionTeams.GetSpawnPoistionRandomAtTeam(roomPlayer.GetComponent<NetworkRoomPlayerExt>().team_m);
            gamePlayer = startPos != null
                ? Instantiate(playerPrefab, startPos.position, startPos.rotation)
                : Instantiate(playerPrefab, Vector3.zero, Quaternion.identity);
        }

        GameObject gamePlayer_ = Instantiate(characters[roomPlayer.GetComponent<NetworkRoomPlayer>().Character].GameplayCharacterPrefab, 
            startPos.position == null ? Vector3.zero : startPos.position, Quaternion.identity);
